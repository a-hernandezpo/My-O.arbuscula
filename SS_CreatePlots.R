...
p <- ggplot(df_volc, aes(x = log2FoldChange, y = -log10(padj), color = threshold)) +
  ...
  labs(
    #title = "SS Volcano plot: Sym vs Apo",
...
ggsave("./SS figures/SS Volcano_Sym_vs_Apo_COMPLETO.png", 
       plot = p, 
...
pheatmap(degs_plot[,1:10], show_rownames=T, labels_row=degs_plot$genename,
         ...
         filename="./SS figures/heatmap_DEGs_annotonly.png", width=8, height=15)
...
pheatmap(glut_degs[,1:10], 
         ...
         filename="./SS figures/glut_DEGs.png", width=5, height=5)
...
pheatmap(sug_lip_degs[,1:10], 
         ...
         color=ccol, cutree_rows = 2, cutree_cols = 2, 
         treeheight_col = 5, treeheight_row = 5, legend=T,
         filename="./SS figures/sug_lip_DEGs.png", width=5, height=5)
...
ggsave("./SS figures/Fig2_new.png", dpi=300, height=6, width=10, units="in")
